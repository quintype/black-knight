...
if [ $replicas -gt 1 ];then
  echo "aborting: some existing dpeloyment is going on which have one or more pod in ready state"
  exit 1
fi

ready=$(kubectl get rc $next_deployment_id --namespace=$username --server=$KUBE_MASTER | tail -n+2 | awk '{print $4}')
if [ $ready -eq 1 ];then
  echo "aborting: some existing dpeloyment is going on which have one pod in ready state"
  exit 1
fi
...
if [ $age_in_second -lt $TIMEOUT ];then
   echo -e "aborting: some existing dpeloyment is going on either wait for it to complete or timeout\n time reamining for TIMEOUT is: $(( $TIMEOUT - $age_in_second))"
   exit 1
fi
