...
if [ "$ABORT" -eq 1 ]; then
  ...
  $kubecmd run -i ${tag} --image=${repo}:${tag} --labels app=${app_name}-migrate --restart=Never $secrets_string --overrides='{"spec":{"imagePullSecrets":[{"name":"myregistrykey"}],"nodeSelector": {"failure-domain.beta.kubernetes.io/zone": "us-east-1d","minion_role": "all"}}}' --command -- $@
  $kubecmd delete pod ${tag}
fi
