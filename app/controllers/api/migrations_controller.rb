class Api::MigrationsController < ApplicationController
  ...
  def destroy
    ...
    render json: {message: `KUBE_MASTER=#{cluster.kube_api_server} ABORT=1 ./bin/docker-migration.sh #{deploy_env.publisher.username} #{deploy_env.repository} #{migration.deploy_tag} #{deploy_env.app_name} #{deploy_env.migrate_with_sidecar} 2>&1`}
  end
end
