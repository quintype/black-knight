class Api::LogsController < ApplicationController
  ...
  def show
    ...
    if deploy_environment.running_pods.include?(params[:pod]) && deploy_environment.log_files.include?(params[:log_file])
      ...
      respond_with log_output: `KUBE_MASTER=#{deploy_environment.cluster.kube_api_server} ./bin/kube-status logs #{pod_name} #{log_file} #{lines} #{deploy_environment.username} 2>&1`
    else
