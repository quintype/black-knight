class DeployContainerJob < ApplicationJob
  ...
  def post_slack(deployment)
    ...
    message = {building: "Building #{environment.app_name} #{environment.name} with tag #{deployment.version}", success: "Deployed #{environment.app_name} with tag #{deployment.deploy_tag}. deploy_container_job.test", deploying: "Build successful. Deploying #{environment.app_name} #{environment.name} with tag #{deployment.version}"}
